...
public class FileSystemHandlerImpl extends DefaultCommHandler implements FileSystemHandler
{
     ...
     */
    @Override
    public FileSystemIterator getFileSystemByStoragePool(String storagePoolId, FileSystemQuery condition)
        throws ApiException
    {
        if (null == condition)
        {
            FileSystemMO mo = new FileSystemMO();
            mo.setAssociateOBJId(storagePoolId);
            mo.setAssociateOBJType(MOType.STORAGEPOOL);
            condition = new FileSystemQuery(mo, 0, 100);
            return getBatch(condition);
        }
        FileSystemMO mo = condition.getFileSystemMO();
        if (null == mo)
        {
            mo = new FileSystemMO();
        }
        mo.setAssociateOBJId(storagePoolId);
        mo.setAssociateOBJType(MOType.STORAGEPOOL);
        condition.setQueryFileSystemMO(mo);
        return getBatch(condition);
    }
