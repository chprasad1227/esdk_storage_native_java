...
public class HomeDirHandlerImpl extends DefaultCommHandler implements
    HomeDirHandler
{
    private HomeDirIterator iterator = null;
     ...
     */
    @Override
    public HomeDirMO get(String id) throws ApiException
    {
        String realitivePath = OpenApiUtils.getOpenApiUtilsInstance()
            .composeRelativeUri(getDeviceId(), "homedir", id);
     ...
     */
    @Override
    public HomeDirIterator getBatch(HomeDirQuery condition)
    {
        if (null == iterator)
        {
            iterator = new HomeDirIterator(getRestRequestHandler(),
