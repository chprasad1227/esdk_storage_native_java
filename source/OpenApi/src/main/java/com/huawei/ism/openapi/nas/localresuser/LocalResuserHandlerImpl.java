...
public class LocalResuserHandlerImpl extends DefaultCommHandler implements LocalResuserHandler
{
    ...
    public LocalResuserIterator getBatch(LocalResuserQuery condition)
    {
        if (null == iterator)
        {
            iterator = new LocalResuserIterator(getRestRequestHandler(), getDeviceId(), condition);
        }
        return iterator;
    }
    ...
    public LocalResuserMO create(LocalResuserMO mo) throws ApiException
    {
        ...
        if (null != mo)
        {
            ...
            LogUtil.getLogUtilInstance().showInterfaceLog(LOG_TYPE_E.LOG_INFO, 2,
                    "LocalResuserHandlerImpl.create()", null, properties);
        }
     ...
     */
    @Override
    public LocalResuserIterator getLocalResuserByLocalResGroup(String localResuserGroupNames,
        LocalResuserQuery condition) throws ApiException
    {
        if (null == condition)
        {
            LocalResuserMO mo = new LocalResuserMO();
            mo.setGroupNames(new String[]
            {localResuserGroupNames});
