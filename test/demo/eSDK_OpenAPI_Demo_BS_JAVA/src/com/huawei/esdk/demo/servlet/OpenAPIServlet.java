...
public class OpenAPIServlet extends HttpServlet
{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        {
            resp = getSystemInfo(request);
        }
        else if ("getAlarmInfo".equals(methodName))
        {
            resp = getAlarmInfo(request);
        }
    ...
    private String getAlarmInfo(HttpServletRequest request)
    {
        String lang = request.getParameter("lang");
        if ("en-us".equals(lang))
        {
            manager.setAcceptLanguage(ConstantsDefine.AcceptLanguage.LAN_EN);
        }
        else
        {
            manager.setAcceptLanguage(ConstantsDefine.AcceptLanguage.LAN_CH);
        }
        // 清空告警缓存列表以及初始索引值
        alarms.clear();
        alarmIndex = 0;
    ...
    private String getSystemInfo(HttpServletRequest request)
    {
        String lang = request.getParameter("lang");
        if ("en-us".equals(lang))
        {
