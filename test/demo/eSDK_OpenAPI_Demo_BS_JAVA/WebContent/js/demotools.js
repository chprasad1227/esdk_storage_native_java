function changeLang() {
	...
	var createMain = document.getElementById("create_main");
	if (null != createMain) {
		if (lang == "en-us") {
			createMain.style.background = "url(../image/create_main_en.png)";
		} else {
			createMain.style.background = "url(../image/create_main.png)";
		}
	}
	...
	var arrayOfA;
	if (document.all || document.getElementById) {
		arrayOfA = document.getElementsByTagName("a");
	}

	for ( var i = 0; i < arrayOfA.length; i++) {
		var aId = arrayOfA[i].id;
		if (aId != "" && aId.indexOf("sourceCode") != -1) {
			document.getElementById(aId).title = controller.language.titles[aId];
		}
	}
	...
	var arrayOfA;
	if (document.all || document.getElementById) {
		arrayOfA = document.getElementsByTagName("div");
	}

	for ( var i = 0; i < arrayOfA.length; i++) {
		var aId = arrayOfA[i].id;
		if (aId != "" && aId.indexOf("div") != -1) {
			document.getElementById(aId).innerHTML = controller.language.titles[aId];
		}
	}
